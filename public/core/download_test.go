...
func TestAsinDownload(t *testing.T) {
	util.MakeDir(lizard.Dir + "/test/asin/")
	...
	for {
		...
		for {
			...
			if err != nil {
				...
				util.SaveToFile(lizard.Dir+"/test/asin/"+prefix+util.IS(testtimes)+".html", content)
				break
			}
		}
	}
...
func TestListDownload(t *testing.T) {
	util.MakeDir(lizard.Dir + "/test/list/")
	ip := "*a"
	// debug info will no appear |nothing
	spider.SetLogLevel("info")
	...
	util.SaveToFile(lizard.Dir+"/test/list/jp.html", content)
	...
	util.SaveToFile(lizard.Dir+"/test/list/usa.html", content)
	...
	util.SaveToFile(lizard.Dir+"/test/list/uk.html", content)
	...
	util.SaveToFile(lizard.Dir+"/test/list/de.html", content)
}
