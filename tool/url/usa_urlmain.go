...
func main() {
	if lizard.Local {
		core.InitConfig(lizard.Dir+"/config/"+"usa_local_config.json", lizard.Dir+"/config/"+"usa_log.json")
	} else {
		core.InitConfig(lizard.Dir+"/config/"+"usa_config.json", lizard.Dir+"/config/"+"usa_log.json")
	}
	proxy = lizard.ToolProxy
	//6级别
	...
	switch lizard.ToolStep {
	case 1, 2, 3, 4:
		Good(lizard.ToolStep)
